...
pub async fn compress_video(
    ...
    let args = build_video_args(&preset, &hw_info, &input_path, &temp_path, true);
    ...
    std::fs::rename(&temp_path, &output_path)
        .map_err(|e| AppError::Other(format!("Failed to move output file: {e}")))?;
...
pub async fn cancel_job(
    ...
    let pid = active_jobs.0.lock().unwrap().get(&job_id).copied();
    if let Some(pid) = pid {
        kill_process(pid);
    }
    Ok(())
